...
function validateTitleButtonSafeZone(common, rule, matchedVariant) {
  ...
  const aiTextComponents = getTextAnalysisComponents(common.meta, rule);
  if (aiTextComponents) {
    const error = getTextAnalysisError(common.meta);
    if (!aiTextComponents.length) {
      const recognizedText = getRecognizedTextSummary(common.meta);
      return {
        ...common,
        status: 'warn',
        current: error
          ? `文字识别失败：${error}`
          : recognizedText
            ? `文字识别到文案但没有有效坐标：${recognizedText}`
            : '未识别到标题 / 按钮坐标',
        required,
        tip: error || common.tip || '请确认 文字识别接口可用，或人工复核文字和按钮位置'
      };
    }
    ...
    return {
      ...
      current: offenders.length
        ? `文字识别 ${formatBounds(badBounds, common.meta)}（超出安全区）`
        : `文字识别 ${formatBounds(checkedBounds, common.meta)}（位于安全区）`,
      required,
      tip: offenders.length ? common.tip : undefined,
      markers: offenders.length ? createMarkersForComponents(offenders, '文字识别超出安全区') : []
    };
  }
...
function validateTextSafety(common, rule, matchedVariant) {
  ...
  const usingAiText = hasTextAnalysis(common.meta);
  if (!textComponents.length) {
    ...
    return {
      ...
      tip: error || common.tip || (usingAiText ? '请人工复核 未识别出的文字和按钮位置' : '当前为启发式图像检测，请人工复核文字和按钮位置')
    };
  }
  ...
  return {
    ...
    status: offenders.length ? (rule.level === 'warning' ? 'warn' : 'fail') : 'pass',
    current: offenders.length
      ? `${usingAiText ? '文字识别文字 / 按钮超出安全区' : '文字 / 按钮疑似进入危险区'}：${formatBounds(badBounds, common.meta)}`
      : `${usingAiText ? '文字识别文字 / 按钮位于安全区' : '文字 / 按钮位于安全区'}：${formatBounds(checkedBounds, common.meta)}`,
    required: rule.requiredText || '文字和按钮需位于安全区内',
    tip: offenders.length ? common.tip : undefined,
    markers: offenders.length
      ? (usingAiText ? createMarkersForComponents(offenders, '文字识别超出安全区') : [createMarker(badBounds, '文字/按钮超出安全区')])
      : []
  };
}
...
function getTextLikeComponents(meta, rule = {}) {
  const aiTextComponents = getTextAnalysisComponents(meta, rule);
...
function hasTextAnalysis(meta) {
  return !!meta?.textAnalysis && Array.isArray(meta.textAnalysis.texts);
}

function getTextAnalysisError(meta) {
  return String(meta?.textAnalysis?.error || '').trim();
}

function getRecognizedTextSummary(meta) {
  const texts = Array.isArray(meta?.textAnalysis?.texts) ? meta.textAnalysis.texts : [];
  return texts
...
function getTextAnalysisComponents(meta, rule = {}) {
  if (!hasTextAnalysis(meta) || getTextAnalysisError(meta)) return null;
  const allowedTypes = new Set((rule.textTypes || ['text', 'button', 'number']).map(type => String(type).toLowerCase()));
  const components = meta.textAnalysis.texts
